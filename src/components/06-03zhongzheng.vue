<template>
  <div ref="draggable" class="container">
    <img v-show="showMap" ref="image" src="../assets/06-03zhongzheng/test.png" class="map" @load="onImageLoad">
    <svg
        class="svg-element"
      ...
      <circle
          ...
          v-if="robot.map === '06-03' && robot.outElevator"
          :key="robot.id"
...
export default {
  ...
  mounted() {
    ...
    const map_data = this.$store.state.map_data[this.$store.state.map_name.indexOf('06-03')];
