<template>
  <div ref="draggable" class="container">
    <img v-show="showMap" ref="image" src="../assets/B1/test.png" class="map" @load="onImageLoad">
    <svg
        class="svg-element"
      ...
      <circle
          v-for="robot in $store.state.robot_data.filter(r => r.map === 'B1')"
...
export default {
  ...
  mounted() {
    ...
    const map_data = this.$store.state.map_data[this.$store.state.map_name.indexOf('B1')];
