<template>
  <div ref="draggable" class="container">
    <img v-if="showMap" ref="image" src="../assets/B04-01-menzhen/test.png" class="map" @load="onImageLoad">
    <svg
        class="svg-element"
      ...
      <path
          ...
          :d="getPathD(path)"
          stroke="gray"
          :stroke-width="!fullScreen ? 2 : 3"
          v-if="visible_passInfo">
      </path>
      <path :d="getPassedPath()" stroke="gray" :stroke-width="!fullScreen ? 2 : 3" v-if="visible_passInfo"></path>
      <path :d="getUnPassedPath()" stroke="black" :stroke-width="!fullScreen ? 2 : 3" v-if="visible_passInfo"></path>
      <path
      ...
      <circle
          v-for="robot in $store.state.robot_data.filter(r => r.map === 'B04-01')"
          v-if="robot.outElevator"
          :key="robot.id"
          :cx="`${robot.x/map_width*100}%`"
          :cy="`${robot.y/map_height*100}%`"
          :r="!fullScreen ? 1.5 : 4"
          fill="black"
          @click="showInfo('robot', robot)"
          @dblclick="hideInfo('robot', robot)">
      </circle>
    </svg>
    <!--节点信息的描述列表-->
    <el-descriptions
        border
        direction="vertical"
        :style="{left: target_node.x + 100 +'px', top: target_node.y - 100 + 'px'}"
    ...
    <el-descriptions
        border
        :column="1"
        :style="{left: target_path_midX + 100 +'px', top: target_path_midY - 100 + 'px'}"
...
export default {
  ...
  mounted() {
    ...
    const map_data = this.$store.state.map_data[this.$store.state.map_name.indexOf('B04-01')];
