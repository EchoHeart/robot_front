<template>
  <div ref="draggable" class="container">
    <img v-show="showMap" ref="image" src="../assets/02-03/test.png" class="map" @load="onImageLoad">
    <svg
        class="svg-element"
    ...
    <el-descriptions
        border
        direction="vertical"
        :style="{left: target_node.x + 200 +'px', top: target_node.y - 100 + 'px'}"
    ...
    <el-descriptions
        border
        :column="1"
        :style="{left: target_path_midX + 200 +'px', top: target_path_midY - 100 + 'px'}"
...
export default {
  ...
  mounted() {
    ...
    const map_data = this.$store.state.map_data[this.$store.state.map_name.indexOf('02-03')];
