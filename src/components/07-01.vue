<template>
  <div ref="draggable" class="container">
    <img v-if="showMap" ref="image" src="../assets/07-01/test.png" class="map" @load="onImageLoad">
    <svg
        class="svg-element"
      ...
      <circle
          ...
          v-if="robot.map === '07-01' && robot.outElevator"
          :key="robot.id"
...
export default {
  ...
  mounted() {
    ...
    const map_data = this.$store.state.map_data[this.$store.state.map_name.indexOf('07-01')];
