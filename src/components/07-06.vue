<template>
  <div ref="draggable" class="container">
    <img v-if="showMap" ref="image" src="../assets/07-06/map.png" class="map" @load="onImageLoad">
    <svg
        class="svg-element"
      ...
      <circle
          ...
          :cx="`${node.x}%`"
          :cy="`${node.y}%`"
          :r="!fullScreen ? 0.3 : (node.is_target ? 3 : 1.5)"
      ...
      <circle
          ...
          v-if="robot.map === '07-06' && robot.outElevator"
          :key="robot.id"
...
<!--    &lt;!&ndash;机器人信息的描述列表&ndash;&gt;-->
<!--    <el-descriptions-->
<!--        border-->
<!--        direction="vertical"-->
<!--        :style="{left: target_robot.x + 10 +'px', top: target_robot.y + 10 + 'px'}"-->
<!--        v-show="visible_robotInfo">-->
<!--      <el-descriptions-item label="机器人ID">{{ target_robot.id }}</el-descriptions-item>-->
<!--      <el-descriptions-item label="X坐标">{{ target_robot.x.toFixed(2) }}</el-descriptions-item>-->
<!--      <el-descriptions-item label="Y坐标">{{ target_robot.y.toFixed(2) }}</el-descriptions-item>-->
<!--      <el-descriptions-item label="备注">-->
<!--        <el-tag type="info">机器人</el-tag>-->
<!--      </el-descriptions-item>-->
<!--    </el-descriptions>-->
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    const map_data = this.$store.state.map_data[this.$store.state.map_name.indexOf('07-06')];
