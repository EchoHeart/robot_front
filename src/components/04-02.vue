<template>
  <div ref="draggable" class="container">
    <img v-if="showMap" ref="image" src="../assets/04-02/test.png" class="map" @load="onImageLoad">
    <svg
        class="svg-element"
    ...
    <el-descriptions
        ...
        ref="nodeInfo"
        direction="vertical"
        :style="{left: target_node.x - 310 +'px', top: target_node.y - 100 + 'px'}"
    ...
    <el-descriptions
        border
        :column="1"
        :style="{left: target_path_midX - 280 +'px', top: target_path_midY - 100 + 'px'}"
        v-show="visible_pathInfo && fullScreen">
      <el-descriptions-item label="路径ID">{{ target_path.id }}</el-descriptions-item>
      <el-descriptions-item label="起点节点ID" :contentStyle="{'white-space': 'pre-wrap'}">{{ target_path.startAddress }}</el-descriptions-item>
      <el-descriptions-item label="终点节点ID" :contentStyle="{'white-space': 'pre-wrap'}">{{ target_path.endAddress }}</el-descriptions-item>
      <el-descriptions-item label="类型">
...
export default {
  ...
  mounted() {
    ...
    const map_data = this.$store.state.map_data[this.$store.state.map_name.indexOf('04-02')];
  ...
  methods: {
    ...
    showInfo(flag, t){
      if(flag === 'node'){
        ...
        this.target_node.name = this.handleText(t.name, 12);
        ...
        this.target_path.startAddress = this.handleText(s.name, 12);
        this.target_path.endAddress = this.handleText(e.name, 12);
